...
class FlashcardRepository(
    ...
    suspend fun autoGenerateDeckByStatus(status: LearningStatus): Long {
        val vocabEntities = when (status) {
            LearningStatus.NOT_LEARNED -> vocabDao.getVocabsByStatus("NOT_LEARNED")
            LearningStatus.LEARNED -> vocabDao.getVocabsByStatus("LEARNED")
        }
        
        // Get first emission from flow
        val vocabs = vocabEntities.map { entities ->
            entities.map { it.toDomain() }
        }.first()
        
        val deckName = when (status) {
            LearningStatus.NOT_LEARNED -> "Từ chưa học"
            LearningStatus.LEARNED -> "Từ đã học"
        }
        
        val description = when (status) {
            LearningStatus.NOT_LEARNED -> "Bộ thẻ tự động tạo từ vựng chưa học"
            LearningStatus.LEARNED -> "Bộ thẻ tự động tạo từ vựng đã học"
        }
    ...
    suspend fun autoGenerateReviewAllDeck(): Long {
        val vocabs = vocabDao.getAllVocabs().map { entities ->
            entities.map { it.toDomain() }
        }.first()
        ...
        return autoGenerateDeckFromVocab(
            vocabs,
            "Ôn tất cả từ vựng",
            "Bộ thẻ tự động tạo chứa tất cả từ vựng của bạn"
        )
    }
